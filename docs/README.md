...
`pytest_cache_assert` will automatically create: `tests/cache-assert/source_file/test_file/test_create_data-00.json` (and `test_create_data-01.json`) for each of the parameters when first run by caching the `result`. Below is the example for `test_create_data-00.json`. The cached files should be checked into version control. They can be manually edited if needed or regenerated by deleting the file and re-running the tests suite.
